...
class NeuralNet:
    ...
    def leaky_relu(self, alpha, u):
        ...
        """
        lrelu = np.copy(u)
        return np.maximum(lrelu, alpha*lrelu)
    ...
    def softmax(self, u):
        ...
        o = u_exp / np.sum(u_exp, axis=1, keepdims=True)
    ...
    def _dropout(u, prob):
        ...
        """
        # TODO
        dropout_mask = np.random.rand(*np.shape(u)) >= prob
    ...
    def update_weights(self, dLoss, use_momentum):
        ...
        for layer in dLoss:
            if use_momentum:
                ...
                self.parameters[layer] = self.parameters[layer] - self.learning_rate * self.change[layer]
            else:
                self.parameters[layer] = self.parameters[layer] - self.learning_rate * dLoss[layer]
    ...
    def compute_gradients(self, y, yh, use_dropout):
        ...
        dLoss_u2 = (yh-y)/ np.shape(y)[0]
        dLoss_theta2 =  self.cache['o1'].T@dLoss_u2
        dLoss_b2 = np.sum(dLoss_u2, axis=0, keepdims=False)
        dLoss_o1 = (self.parameters['theta2']@dLoss_u2.T).T
        ...
        print(dLoss_theta1)
        dLoss_b1 = np.sum(dLoss_u1, axis=0, keepdims = True)
        #print(dLoss_theta2)
        gradients = {
            "theta1": dLoss_theta1,
            "b1": dLoss_b1,
    ...
    def gradient_descent(self, x, y, iter=60000, use_momentum=False, local_test=False):
        ...
        **For LOCAL TEST append and print out loss at every iteration instead of every 1000th multiple.
        ...
        for i in range(iter):
            ...
            self.backward(y, yh, self.use_dropout, use_momentum=use_momentum)
    ...
    def batch_gradient_descent(self, x, y, use_momentum, iter=60000, local_test=False):
        ...
        for i in range(iter):
            # TODO: implement training loop
            start_idx = i * self.batch_size % len(x)
            end_idx = (i * self.batch_size + self.batch_size) % len(x)
