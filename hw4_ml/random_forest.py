...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        row_idx =  np.random.choice(num_training, num_training, replace = True) # TODO
        #################################################################################
        ############# Get Col Indices Second - write your code below ####################
        col_idx = np.random.choice(num_features, int(self.max_features *num_features), replace = False) # TODO
        ##################################################################################
    ...
    def fit(self, X, y):
        ...
        # Initialize bootstrap datasets
        self.bootstrapping(num_training, num_features)

        # Train decision trees
        for i in range(self.n_estimators):
            # Use the row and column indices for training
    ...
    def plot_feature_importance(self, data_train):
        ...
        for tree in self.decision_trees:
            ...
            plt.title("Feature importances of tree " + str(i))
            plt.bar(data_train.columns[indices], importances[indices])
            plt.xlabel("features")
    ...
    def select_hyperparameters(self):
        ...
        """
        n_estimators = 11 # TODO
        max_depth = 12 # TODO
        max_features = 0.6 # TODO
